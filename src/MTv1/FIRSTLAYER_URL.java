...
//                    System.out.println(res_segment[i]);
        ...
        }else if(FirstLayerRes.contains("#EXTINF:")){
...
//        System.out.println(FirstLayerRes.lines());
        ...
        if (m3u8_link!=null){
            if (m3u8_link=="0"){  // get chunklist -> return original url
                return m3u8;

            }else{ // get playlist -> return second layer content
                String base_link = m3u8.substring(0, m3u8.lastIndexOf("/")) ;
                System.out.println(base_link+"/"+m3u8_link);
                return base_link+"/"+m3u8_link;
            }
        }else{  // -> return is null -> incorrect url
            throw new IOException("wrong url cannot download");
        }
