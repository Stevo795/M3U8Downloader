...
//        String DOWNLOAD_PATH = "C:\\Users\\STEVO\\Desktop\\java_project\\Blob\\t5\\";
        ...
        List<MultiThread> Thread_List = new ArrayList<MultiThread>();
...
//        final CountDownLatch latch1 = new CountDownLatch(4);
        ...
        //通用 file task per t
        if (Thread_num == 4) {
            thread1 = new MultiThread("thread1", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread1", task_per_t, task_per_t * 0);
            thread1.start();


            //
            thread2 = new MultiThread("thread2", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread2", task_per_t, task_per_t * 1);
            thread2.start();

            //
            thread3 = new MultiThread("thread3", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread3", task_per_t, task_per_t * 2);
            thread3.start();

            //
            thread4 = new MultiThread("thread4", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread4", task_per_t + task_res, task_per_t * 3);
            thread4.start();
        ...
        if (Thread_num == 6) {
            thread1 = new MultiThread("thread1", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread1", task_per_t, task_per_t * 0);
            thread1.start();
            thread2 = new MultiThread("thread2", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread1", task_per_t, task_per_t * 1);
            thread2.start();
            thread3 = new MultiThread("thread3", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread1", task_per_t, task_per_t * 2);
            thread3.start();
            thread4 = new MultiThread("thread4", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread1", task_per_t , task_per_t * 3);
            thread4.start();
            thread4 = new MultiThread("thread5", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread1", task_per_t , task_per_t * 4);
            thread4.start();
            thread4 = new MultiThread("thread6", (ArrayList) ts_fregmetns_list, DOWNLOAD_PATH_FILE, "thread1", task_per_t + task_res, task_per_t * 5);
            thread4.start();
        }
